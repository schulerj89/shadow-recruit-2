...
- **Wall-run gate:** require a sorted coordinate ledger for every wall line interrupted by doors. The ledger must prove door-to-wall, door-to-frame, continuity/back-wall, and door-to-door spans are owned in open and closed states before the slice can be called fixed.
- **Title staging gate:** ask `$shadow-recruit-game-designer` and `$shadow-recruit-game-tester` before approving title-screen work. The hero must face/read toward the camera, occupy meaningful screen space, and be staged as the primary subject rather than appearing as a tiny actor in a zoomed-out Level 1 preview.
- **AAA density gate:** ask `$threejs-level-world-builder` and `$threejs-aaa-asset-builder` before approving a large mission space that lacks measured prop/set-dressing density, landmarks, cover language, generated texture variation, and focal GLB assets. A big floor plan with repeated wall/floor textures is still a blockout until foreground, midground, and background gameplay detail read from the camera.
- **Tester escalation gate:** when QA misses an obvious screenshot issue, update the tester rubric or debug instrumentation immediately. The next loop must require the screenshot name plus coordinates/projection/density data that would have made the issue measurable.
